...
/**
 * Lists step definitions related to keyboard commands on the UI page elements.
 * 
 * Example: 
 * <blockquote><pre>
 *   > ["CONTROL", "A"] to select all text in a specified page element.
...
public class SmartUiKeyboardOperationsStepDefs {
	...
	public void use_keys_on_page_object_to_perform_operation(String keys, String pe, String operation) {
		...
		DOMObjectValidator domObjectValidator = (DOMObjectValidator) PageObjectUtil.invokeValidatorMethod("getDOMObjectValidator", new String[] {},
				new Object[] { }, peInfo, scenarioContext);
		
		domObjectValidator.sendKeys(peInfo.getMaxIterationsToLocateElements(), Keys.chord(keysChord));
	}
	...
	public void use_keys_and_mouse_event_together_on_page_object_to(String keys, String mouseEvent, String pe, String operation) {
		if(!scenarioContext.isLastConditionSetToTrue()) {
			scenarioContext.log("This step is not executed due to false value of condition=\"" + scenarioContext.getLastConditionName() + "\".");
			return;
		}
			...
			try {
				...
				if(i == (keyList.size() - 1)) {
					...
					PageObjectUtil.invokeValidatorMethod("performKeyDown", new String[] { Keys.class.getTypeName(), Integer.TYPE.getTypeName() },
						new Object[] { selKey, peInfo.getMaxIterationsToLocateElements() }, peInfo,
						scenarioContext);
